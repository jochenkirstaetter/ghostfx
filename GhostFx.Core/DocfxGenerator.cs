...
public static class DocfxGenerator
{
    private const string DefaultMasterTemplate = """
                                                 ...
                                                 <html {{#_lang}}lang="{{_lang}}"{{/_lang}}>
                                                   <head>
                                                     ...
                                                     <link rel="stylesheet" href="{{_rel}}public/ghostfx.css">
                                                   </head>
        ...
        List<BlogPostMetadata>? posts = null)
    {
                ...
                if (node != null)
                {
                    ...
                    if (buildNode != null)
                    {
                        ...
                        if (globalMetadataNode != null)
                        {
                            ...
                            globalMetadataNode["_disableAffix"] = config.DisableAffix;
        ...
        var globalMetadata = new Dictionary<string, object>
        {
            ...
            ["_disableAffix"] = config.DisableAffix,
                    ...
                    else if (hbsNameLower == "floating-header")
                    {
                        converted = """
                                    <div class="floating-header">
                                        ...
                                        {{#_enableSearch}}
                                        <div class="floating-header-search">
                                            <input class="form-control" id="search-query-floating" type="search" placeholder="Search" autocomplete="off" aria-label="Search">
                                        </div>
                                        {{/_enableSearch}}
                                    ...
                                    (function () {
                                        function initFloatingHeader() {
                                            ...
                                            try {
                                                ...
                                                // Sync search inputs
                                                (function () {
                                                    var primarySearch = document.getElementById('search-query');
                                                    var secondarySearch = document.getElementById('search-query-floating');
                                                    if (!primarySearch || !secondarySearch) return;

                                                    secondarySearch.addEventListener('input', function() {
                                                        primarySearch.value = secondarySearch.value;
                                                        primarySearch.dispatchEvent(new Event('input', { bubbles: true }));
                                                    });

                                                    primarySearch.addEventListener('input', function() {
                                                        secondarySearch.value = primarySearch.value;
                                                    });

                                                    var observer = new MutationObserver(function() {
                                                        secondarySearch.disabled = primarySearch.disabled;
                                                        if (primarySearch.placeholder) {
                                                            secondarySearch.placeholder = primarySearch.placeholder;
                                                        }
                                                    });
                                                    observer.observe(primarySearch, { attributes: true, attributeFilter: ['disabled', 'placeholder'] });
                                                    secondarySearch.disabled = primarySearch.disabled;
                                                    if (primarySearch.placeholder) {
                                                        secondarySearch.placeholder = primarySearch.placeholder;
                                                    }
            ...
            string schemaContent = @"<script type=""application/ld+json"">
{
    ...
    {{/_enableSearch}}";
        ...
        result = Regex.Replace(result, @"\{\{\s*body_class\s*\}\}", "tex2jax_ignore {{bodyClass}}{{#_disableAffix}} metadata-disable-affix{{/_disableAffix}}",
            RegexOptions.IgnoreCase);
        ...
        result = Regex.Replace(result, @"\{\{\{\s*body\s*\}\}\}", "{{!body}}\n{{^_disableAffix}}\n<div class=\"affix\">\n  <nav id=\"affix\"></nav>\n</div>\n{{/_disableAffix}}", RegexOptions.IgnoreCase);
        result = Regex.Replace(result, @"\{\{\s*body\s*\}\}", "{{!body}}\n{{^_disableAffix}}\n<div class=\"affix\">\n  <nav id=\"affix\"></nav>\n</div>\n{{/_disableAffix}}", RegexOptions.IgnoreCase);
        ...
        if (isLayout)
        {
            ...
            if (!result.Contains("public/ghostfx.css", StringComparison.OrdinalIgnoreCase))
            {
                result = Regex.Replace(result, @"(<link\s+[^>]*href=[""'][^""']*public/main\.css[""'][^>]*>)",
                    "$1\n    <link rel=\"stylesheet\" href=\"{{_rel}}public/ghostfx.css\">",
                    RegexOptions.IgnoreCase);
            }

            if (!result.Contains("metadata-disable-affix"))
            {
                if (result.Contains("<body", StringComparison.OrdinalIgnoreCase))
                {
                    if (Regex.IsMatch(result, @"<body[^>]*class=", RegexOptions.IgnoreCase))
                    {
                        result = Regex.Replace(result, @"<body([^>]*class=[""'])([^""']*)([""'])",
                            $"<body$1$2{{#_disableAffix}} metadata-disable-affix{{/_disableAffix}}$3",
                            RegexOptions.IgnoreCase);
                    }
                    else
                    {
                        result = Regex.Replace(result, @"<body([^>]*)>",
                            $"<body$1 class=\"{{#_disableAffix}}metadata-disable-affix{{/_disableAffix}}\">",
                            RegexOptions.IgnoreCase);
                    }
                }
            }
        ...
        List<BlogPostMetadata>? pages = null, List<BlogPostMetadata>? posts = null)
    {
        ...
        string ghostfxCssPath = Path.Combine(publicDir, "ghostfx.css");
        string defaultGhostfxCss = await ReadEmbeddedGhostfxCssAsync();
        await File.WriteAllTextAsync(ghostfxCssPath, defaultGhostfxCss, System.Text.Encoding.UTF8);
    ...
    private static async Task<string> ReadEmbeddedGhostfxCssAsync()
    {
        var assembly = typeof(DocfxGenerator).Assembly;
        using var stream = assembly.GetManifestResourceStream("GhostFx.Core.ghostfx.css");
        if (stream == null)
        {
            return string.Empty;
        }
